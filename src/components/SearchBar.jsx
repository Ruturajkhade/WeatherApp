        ...
        <div className="relative w-full max-w-2xl" ref={searchRef}>
            <form className="relative" onSubmit={handleSubmit}>
                <div className="relative group">
                    <Search className='absolute left-4 top-1/2 transform -translate-y-1/2 text-gray/60 w-5 h-5 group-focus:focus-within:text-white transition-all' />
                    <input type="text" placeholder='Search for any city worldwide.....'
                        value={query} onChange={(e) => setQuery(e.target.value)}
                        className="w-full pl-12 pr-24 py-4 bg-white/10 backdrop-blur-xl border border-white/20 rounded-2xl text-white placeholder-white-50
            ...
            {showSuggestion && (suggestion.length > 0 || searchLoading) && (
                <div className="absolute top-full left-0 right-0 mt-3 bg-white/10 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl overflow-hidden z-50">
                    ...
                    {searchLoading ? (
                        ...
                        suggestion.map((city, index) => {
                            return (
                                <button className="w-full px-6 py-4 text-left hover:bg-white/10 transition-all duration-200 
